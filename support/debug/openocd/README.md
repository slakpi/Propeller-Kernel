...
```shell
openocd -f support/debug/openocd/interface/ft4232h.cfg -f support/debug/openocd/target/rpi3_jtag.cfg
```

Start the AArch64 debugger for a 64-bit build:

```shell
/opt/cross/gnu-aarch64-none-elf/bin/gnu-aarch64-none-elf-gdb -x support/debug/openocd/aarch64.gdb
```

Or the ARM debugger for a 32-bit build:
