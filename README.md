...
```shell
cargo run --config .cargo/config-qemu.toml \
...
```shell
cargo run --config .cargo/config-rpi3.toml \
          --target armv7a-none-eabi \
          -- --image kernel7.img
```
...
```shell
cargo run --config .cargo/config-rpi4.toml \
          --target aarch64-unknown-none-softfloat \
          -- --image kernel8.img
```
...
Include the `--features` argument for `cargo run` to enable extra features.
...
Copy the DTB files off of a SD card with a clean install of Raspberry Pi OS for debugging.

Use the following QEMU options:

* `-M` to specify the machine (`raspi2b`, `raspi3b`, `raspi4b`)
* `-kernel` to specify the kernel image.
* `-dtb` to specify the flat DeviceTree binary.
* `-serial null -serial stdio` to quiet the main UART and redirect the mini-UART to `STDOUT`.
* `-gdb tcp::9000` to start the GDB server on port 9000.
* `-S` to halt until GDB connects and continues.

Invoke ARM QEMU with:

```shell
qemu-system-arm -M raspi2b -kernel target/armv7a-none-eabi/debug/kernel7.img \
                -dtb <path to DTBs>/bcm2709-rpi-2-b.dtb \
...
```shell
qemu-system-aarch64 -M raspi3b -kernel target/aarch64-unknown-none-softfloat/debug/kernel8.img \
                    -dtb <path to DTBs>/bcm2710-rpi-3-b.dtb \
                    -serial null -serial stdio \
                    -gdb tcp::9000 -S
```

The repo provides GDB scripts to make debug setup a little faster.
