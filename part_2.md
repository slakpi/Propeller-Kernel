...
`rustup` is the easiest way to install Rust and the necessary targets. Once you have `rustup` installed, run the following to install the cross-compiler targets:
...
Note the use of the software floating-point targets. Propeller avoids floating-point operations in the kernel to avoid the need to save floating-point registers during system calls, etc.

The build tooling is also going to need `llvm-tools` for building the final kernel image. Run:
...
`cargo-binutils`, built on `llvm-tools`, installs utilities such as `rust-objcopy` and `rust-objdump` that the build tooling will use to create the kernel image and disassembly listing.
...
The build tooling uses the `cc` crate to compile assembly files and link the kernel. So, let's set up a global Cargo configuration that points the `cc` crate to the appropriate toolchain for a target.
...
The easiest route is to just use Cargo to create a new binary project:

```
$ cargo new --bin propeller
```

This is going to create a binary that depends on the Rust standard library: your run-of-the-mill executable. But, we are not trying to build a run-of-the-mill executable. We want a bare-metal superstar, a master of the processor running it.

## no_std

Go ahead and delete the `main` function Cargo creates in `main.rs` and let's build out the minimal Rust code:
...
The Rust code above tells the compiler to use the dependency-free Rust [`core`](https://doc.rust-lang.org/stable/core/index.html) library instead of the standard library. As noted in the documentation for `core`, it is feature-rich compared to C, but does not include any features that use dynamic memory, concurrency, I/O, etc.

It also tells the compiler not to expect a `main` function. This will be handled by the linker script later.

Finally, the code sets up a custom Panic handler and defines the function `pk_init` with C-linkage so we can call it from assembly.

## Cargo.toml

Update the `Cargo.toml` file to abort on a panic and add `cc` as a build dependency:
...
Do not bother trying to build yet. It will be a while before the project is build-ready.

## Post-Build Python Script

Propeller uses a [post-build script](https://github.com/slakpi/Propeller-Kernel/blob/999066fd38e1c0fac8e593a8dc15306c2ec2d2cd/support/post-build.py) to generate a disassembly listing and final kernel image. You can place this script in `<propeller>/support`.
...
Each target also specifies `post-build.py` as the "runner" for the target. `cargo build` will only run the `build.rs` we are about to create. By setting up `runner` here, `cargo run` will perform the post-build tasks.

Let's also create a configuration file for QEMU. Create another configuration file called `<propeller>/.cargo/config-qemu.toml`.
...
* Start the kernel code at 0xffff_0000_0008_0000. QEMU's boot loader will jump to 0x0000_0000_0008_0000 expecting to find the kernel code, but we need to specify the virtual address here (more on this later).

For ARMv7a:

* NEON intrinsics are not available in the `eabi` toolchain, so that's already taken care of.
* Optimize for the Cortex-A7.
* Start the kernel code at 0xc001_0000. QEMU differs from the Raspberry Pi boot loader here. The Raspberry Pi boot loader will expect the kernel code at 0x0000_8000. Again, specify the virtual address here (more on this later).

The `[build]` section specifies the targets available to build with the `cargo build` command.

The `[env]` section provides the flags used when compiling assembly files with ARM toolchains. They are mostly just GCC equivalents of the Rust flags.

> *NOTE:* `-fPIC` generates position-independent code from the assembly source. This is important because we are telling the linker to use a virtual base address for the kernel. A jump to a virtual address, for example, will jump to Nowhereland when the MMU is off. So, all early boot code must use PC-relative addresses.

> *NOTE:* `-march=armv7ve`? What the French, Toast? For the ARMv7a target, the configuration tells GCC to use the ARMv7ve architecture. This is really just ARMv7-A with Virtualization Extensions, i.e. the Cortex-A7 and Cortex-A53.
...
`ENTRY(_start)` - Tell the linker that `_start` is the kernel entry point.
...
`. = ALIGN(__page_size)` - Moves the current position to a 4 KiB boundary before beginning the next section.

The rest of the script lays out the read-only data (`.rodata`), read-write data (`.data`), and zero-initialized statics (`.bss`).
...
We need to provide Cargo with a build script that tells it how to compile the assembly.
...
Cargo provides the `main` function with the target architecture through an environment variable. Once we know the target architecture, the script instantiates a `cc::Build` object and starts configuring it to compile the assembly.

The last line in `main` tells `cc` to compile the assembly files.

Now add the `configure_for_aarch64` function:
...
Build scripts communicate back to Cargo via `println!`, and next couple of lines tell Cargo to rerun a build if the linker script or any of the assembly files change.
...
This executes `cargo run`, tells it to use the QEMU configuration for AArch64, and passes the `--image kernel8.img` parameter to the Python post-build script.
