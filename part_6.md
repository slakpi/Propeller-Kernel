...
Virtual Memory is a technique that provides user tasks with their own private address spaces, and partitions tasks from each other and the kernel.

At a very high level, modern processors provide Memory Management Units (MMU) that serve as intermediaries between user code and physical memory when enabled. While the MMU is on, all memory addresses are treated as "virtual". When an instruction attempts to read or write memory, the MMU transparently translates the address from virtual to physical using a configuration provided by the kernel. If the translation fails, the MMU raises an exception.

If the kernel has its own configuration and every user tasks has its own configuration, then they can effectively be partitioned from each other by the MMU.

A global MMU configuration called the "split" ([TTBCR.T0SZ](https://arm.jonpalmisc.com/latest_sysreg/AArch32-ttbcr) for ARM and [TCR_EL1.T0SZ/.T1SZ](https://arm.jonpalmisc.com/latest_sysreg/AArch64-tcr_el1) for AArch64) defines the overall Virtual Memory address layout. We briefly touched on Virtual Memory layouts in [Part 3](https://slakpi.github.io/Propeller-Kernel/part_3.html).
...
In the 64-bit example, an address is a virtual kernel address if bits 63:48 are 1. Note in the AArch64 example that an address is a virtual user address only if bits 63:48 are 0. If bits 63:48 are any other combination of 0's and 1's, the address is invalid.

Commonly, processors achieve this split by maintaining two active configurations: one for the kernel and one for the currently running user task. If the required most-significant bits are 1 *and* the processor is running at the appropriate privilege level, the MMU translates the address using the kernel configuration. If the required most-significant bits are 0, the MMU translates the address using the user configuration.

Whenever the kernel preempts a running task and activates another task, it replaces the user configuration with the new task's configuration. Thus every task sees the same *virtual* address space, but the kernel chooses how those virtual addresses translate to *physical* addresses.
...
> NOTE: An exception here is not necessarily a "bad" thing. It could be that the user task or the kernel is attempting to access memory it is not allowed to access. But, in the case of user task, it could be that the task is accessing memory that has been [swapped out](https://en.wikipedia.org/wiki/Memory_paging#Page_faults) and needs to be swapped back.
...
If, instead of a single monolithic table, the MMU translates in stages, a 32-bit system could use a 16 KiB table with 4,096 entries that map 1 MiB sections. The entries in this table could either map the entire 1 MiB section into virtual memory or point to a 1 KiB second-level table with 256 entries that map each 4 KiB page in the 1 MiB section into virtual memory.
...
With [Large Physical Address Extensions](https://developer.arm.com/documentation/den0013/0400/Virtualization/Large-Physical-Address-Extensions?lang=en), ARMv7-A extends this two-level translation into 3 levels.

	Level 1       ->  Level 2       -> Level 3
	4 Entries         512 Entries      512 Entries
	Covers 4 GiB      Covers 1 GiB     Covers 2 MiB

Instead of a 16 KiB Level 1 table and 1 KiB Level 2 tables, all tables are 4 KiB and all entries are 64 bits.

An entry at Level 2 can either map a full 2 MiB section into virtual memory, or point to a Level 3 table that maps 4 KiB pages into virtual memory. When mapping a large contiguous chunk of memory, it can be more space and time efficient to map it using 2 MiB sections and skipping Level 3 translation.

LPAE is also required for a 3:1 split. Propeller will halt if LPAE is not present.

There's an interesting trick that LPAE-enabled ARM processors pull when using a 3:1 split. With a 3:1 split, bits 31:30 must be 1 for a kernel virtual address. Therefore, a Level 1 table is pointless for the kernel page tables and the ARM processor will assume the kernel page tables start at Level 2. Page tables for a user task will still start at Level 1.
...
Like ARM with LPAE, all tables are 4 KiB and all entries are 64 bits.

AArch64 also allows mapping 1 GiB and 2 MiB sections at Levels 2 and 3 respectively.
...
## Propeller's Initial Page Tables

So far, we have been relying on position independent code and relative offsets to work around the fact that our linker script is using virtual addresses, but the MMU has not been configured and enabled. At a minimum, we need to enable the MMU and configure the page tables to map the kernel into virtual memory so that absolute addressing works.
...
1. Find somewhere to store some initial page tables.
2. Configure the page tables to map the kernel and DeviceTree into virtual memory.
3. Configure the MMU's global settings and enable it.

### Step 1: Where do we put the page tables?

We can solve this problem the same way we solved the initial stack problem in Part 5. We can just reserve space in the kernel image for some initial page tables.
...
Let's update the linker script reserve space for 16 page tables.
...
Now consider what we are about to do. We are about to provide the MMU with page tables for the kernel's virtual address space and then turn the MMU on.
...
SECTIONS
{
  ...
  __kernel_id_tables_size = __kernel_id_tables_end - __kernel_pages_start;
...
### Step 2: How do we configure the page tables?
...
> NOTE: We are section-aligning the size of the kernel and the DeviceTree. We are only going to use 2 MiB sections for the initial page tables. So, if the kernel is 2.9 MiB, we will map 4 MiB of address space.
...
We now have all of the variables above needed to generate the page tables.

#### Step 2B: Great, now what actually goes in the page tables?

As described above, AArch64 has 4 levels of translation tables. We are going to assume that the kernel and DTB are fully contained within the first 1 GiB of memory. Furthermore, we are only going to map 2 MiB sections. This means we only need three tables:
...
At a high level, though, the AArch64 pointer entry format that we will use in the Level 1 and 2 tables is:
...
> NOTE: Bits 63:48 of the physical table address are assumed zero because of the address space split. Bits 11:0 of the address are assumed zero because the table must be page-aligned. Only bits 47:12 of the physical address are significant.
...
> NOTE: Like table pointers, bits 63:48 of the section's physical address are assumed zero. Bits 20:0 are assumed zero because the section is aligned to 2 MiB. Only bits 47:21 of the section's physical address are significant.
...
That's all for setting up the page tables. It's a fair amount of assembly code and bit fiddling, but conceptually pretty simple.
...
| 27:26  | b01 (outer cacheability for kernel table walks) |
...
| 11:10  | b01 (outer cacheability for user table walks)   |
| 9:8    | b01 (inner cacheability for user table walks)   |
| 5:0    | b10000 (user region size - 2^[64 - n])          |

System on Chips has a good explanation of [Inner and Outer](https://www.systemonchips.com/armv8-multi-cluster-cache-coherency-and-inner-shareable-memory-configuration/) domains.

> NOTE: It is probably not necessary to allow caching in the outer domain.
...
Rather than setting the same memory attribute bits on every page table entry, `MAIR_EL1` allows creating up to 8 different memory attribute sets that we can reference by index in the page table entries. Right now, we only really need to configure attribute sets for normal and device memory.
...
[`mmu_setup_and_enable`](https://github.com/slakpi/Propeller-Kernel/blob/main/src/arch/aarch64/start/mm.s#L328) does the following:

* Points `TTBR0_EL1` to the Level 1 identity table physical address.
* Points `TTBR1_EL1` to the Level 1 kernel table physical address.
* Sets `TCR_EL1`
* Sets `MAIR_EL1`
* Sets bit 0 in `SCTLR_EL1` to 1 to enable the MMU.
...
OK, and there we are. We've gone virtual!
